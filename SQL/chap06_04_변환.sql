...
/*숫자형 문잘열 인 경우 +연산가능 */
select empno , ename , empno+'500' , empno * '500' ,  empno - '500', empno / '500'
from emp 
where ename = 'ALLEN';
...
select	sysdate,
		...
		to_char(sysdate ,'DDD') as DDD,-- 년 날짜  
		to_char(sysdate ,'DY') as DY, --요일 수 
		to_char(sysdate ,'DAY') as DAY, -- 요일 수요일
		to_char(sysdate ,'WW') as WW, --연중 주 수  
		to_char(sysdate ,'w') as w -- 월중 주 수 
from dual; 
...
select	sysdate,
		to_char(sysdate ,'MM') as MM,
		to_char(sysdate ,'DD') as DD,-- 월 날짜  
		to_char(sysdate ,'DY' ,'NLS_DATE_LANGUAGE=KOREAN') as DY_KOREAN, --요일 수 
		to_char(sysdate ,'DY' ,'NLS_DATE_LANGUAGE=JAPANESE') as DY_JAPANESE, --요일 수 
		to_char(sysdate ,'DY' ,'NLS_DATE_LANGUAGE=ENGLISH') as DY_ENGLISH, --요일 수 
		to_char(sysdate ,'DAY' ,'NLS_DATE_LANGUAGE=KOREAN') as DAY_KOREAN, -- 요일 수요일
		to_char(sysdate ,'DAY' ,'NLS_DATE_LANGUAGE=JAPANESE') as DAY_JAPANESE, -- 요일 수요일
		to_char(sysdate ,'DAY' ,'NLS_DATE_LANGUAGE=ENGLISH') as DAY_ENGLISH -- 요일 수요일
from dual; 
...
select 1300 -'1500','1300'+1500 from dual;
  ...
  /*,가 포함 되어 있어서 숫자로 변환 불가 오류*/
select 1,300 -'1,500' from dual;


--to_number(숫자형 문자열 , 검증포멧 )
...
-- decode(칼럼 , 첫번째조건 , 첫번째 값 , 두번째 조건 , 두번째 값 ,... DEFAULT) , case 
...
-- 급여가 1000 미만이면 1 , 급여가 1000초과 2000미만 2, ...
-- 3000초과 5
...
-- 급여 (sal)이 1000 미만이면 'a' 등급 , 1000이상 2000미만이면 'b'
-- 2000 이상 3000 미만 'c' 3000이상이면 's' 으로 출력 
